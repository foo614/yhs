...
| Docker/VPS deployment shape | Implemented, verified locally | Existing local Compose files remain available for development. `services/api/src/YSHeng.AppHost` publishes the production Docker Compose artifact in CI using pinned Aspire 13.4.6 packages; `infra/docker-compose.aspire.production.yml` layers Docker builds, health checks, restart policies, Caddy, a pinned Aspire dashboard, and a pinned internal OpenTelemetry Collector. The Collector adds production resource attributes and fans logs, metrics, and traces to authenticated Aspire and protected Grafana Cloud OTLP destinations with bounded queues and retries. Caddy routes only `/ops/*` to an internal adapter that preserves the dashboard's `/ops` base path, redirects, and cookie scope; dashboard and Collector have no direct host ports. Contract tests verify PostgreSQL 17, the `postgres_data` volume, service names, connection-string wiring, Collector fan-out, dashboard ingress, and no direct production host ports. Production rollout is manual-dispatch and environment-approval gated. |
...
| Finance/payment tracking | Implemented for V2 non-cash collections; physical-cash V2 linking remains follow-up work | YS Heng sales-invoice PDF generation; server-calculated nett price with audited different-user Boss/Admin variance approval; multiple idempotent partial collections with globally unique active references; collection-linked evidence with strict PDF/image validation; role-restricted audited reconciliation; pending, approved, disbursed, reconciled, and reversed states; reconciled-only balance and delivery-aware sold-state derivation; canonical-buyer locking; legacy compatibility; AutoCount Excel mapping export for manual submission; settlements, spend, commissions, debt recovery, and vouchers. |
| Leads triage | Implemented; focused verification pending | Public enquiries appear in back office, lead/customer linking, status/customer-link filters, duplicate phone reuse, Sales-agent attribution, and a server-scoped Cars I’m Handling view with monthly sold and current-process counts plus the current handoff owner and next action. |
...
| Workflow validations/status changes | Implemented; focused verification pending | Loan moves vehicles to LoanProcessing/private; delivery accepts one active plan, locks buyer/PIC, and keeps terminal records immutable; a Finance V2 receivable is settled only by reconciled collections, while `Sold` requires that clearance plus released delivery; focused tests and the updated smoke path cover the new gates. |
