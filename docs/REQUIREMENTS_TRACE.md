...
| Docker/VPS deployment shape | Implemented, verified locally | Existing local Compose files remain available for development. `services/api/src/YSHeng.AppHost` publishes the production Docker Compose artifact in CI using pinned Aspire 13.4.6 packages; `infra/docker-compose.aspire.production.yml` layers Docker builds, health checks, restart policies, Caddy, and a pinned Aspire dashboard. Caddy routes only `/ops/*` to an internal adapter that preserves the dashboard's `/ops` base path, redirects, and cookie scope; neither has a direct host port. The dashboard uses browser-token access, authenticated OTLP telemetry, and a disabled Telemetry HTTP API. Contract tests verify PostgreSQL 17, the `postgres_data` volume, service names, connection-string wiring, dashboard ingress, and no direct production host ports. Production rollout is manual-dispatch and environment-approval gated. |
...
| Delivery workflow | Implemented, verified | Local list/detail workflow with ordered statuses; server-locked canonical buyer and vehicle; inspection, polish/tint/wash, coverage, notice, outstation, handover, exact delivery evidence, Finance reconciliation release gate, server-owned release audit, and immutable terminal records. |
| Finance/payment tracking | Implemented for V2 non-cash collections; physical-cash V2 linking remains follow-up work | YS Heng sales-invoice PDF generation; server-calculated nett price with audited different-user Boss/Admin variance approval; multiple idempotent partial collections with globally unique active references; collection-linked evidence with strict PDF/image validation; maker-checker reconciliation; pending, approved, disbursed, reconciled, and reversed states; reconciled-only balance and delivery-aware sold-state derivation; canonical-buyer locking; legacy compatibility; AutoCount Excel mapping export for manual submission; settlements, spend, commissions, debt recovery, and vouchers. |
| Leads triage | Implemented, verified | Public enquiries appear in back office with lead/customer linking, status/customer-link filters, duplicate phone reuse, and Sales-agent attribution. The newer Sales My Cars view is outside the selected local Docker slice. |
...
| Requirement | Status | Evidence |
| --- | --- | --- |
| PostgreSQL blob storage | Implemented; focused verification pending | `VehiclePhoto` and `DocumentBlob` bytea columns; workflow ownership includes exact repair, payment, collection, and delivery schedule links; the smoke path is updated for exact delivery ownership. |
...
| Upload size limits | Implemented; focused verification pending | 5 MB photos, 10 MB documents; delivery evidence uses detected PDF/image signatures and collection evidence uses strict PdfPig PDF parsing or bounded image decoding; focused tests and the updated smoke path cover invalid content. |
...
| Workflow validations/status changes | Implemented, verified | Loan moves vehicles to LoanProcessing/private; delivery follows ordered status transitions, keeps terminal records immutable, binds release evidence to the exact delivery/current buyer, and blocks release until Finance reconciliation. Focused tests and the clean-stack smoke cover these gates. |
| Audit trails | Implemented, verified | Mutation audit records with authenticated staff email; public lead audit actor. |
| OCR/AI/WhatsApp/accounting export | OCR and manual accounting export implemented for first scope; other automation remains extension point | Document OCR uses Google Document AI with optional specialized invoice/expense processors and stores extracted draft fields in `OcrJobs`; Finance exports customers, vehicles, sales invoices, collections, purchases, payments, and expenses for staff review and manual AutoCount submission; it is not a verified direct integration. WhatsApp, loan eligibility prediction, photo optimization, and profit prediction remain extension points. |
...
This runs web type-checking, front-office tests, back-office tests, backend tests, Dockerfile contract tests, Docker Compose contract tests, Compose environment validation tests, deployment script contract tests, deployment runbook tests, requirements trace tests, source requirements crosscheck tests, Stitch visual reference handoff tests, production web builds, and the clean local PostgreSQL/API/front/back smoke stack.
