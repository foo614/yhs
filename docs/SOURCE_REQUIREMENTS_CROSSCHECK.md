...
| Delivery booking inspection, schedule, PIC, notification, inspection, prepare document, report | Implemented | The Delivery workboard derives four plain-language stages from a buyer-locked plan, active-staff PIC, schedule, inspection booking/report evidence, and customer notice. |
| Polish, tinted, wash, final checklist | Implemented | The Prepare car and Handover stages track these checks and expose one server-derived next action instead of a long status form. |
| Insurance and road tax | Implemented | The Clear documents stage requires delivery-bound `Policy` and `RoadTaxReceipt` evidence, valid expiry dates, and read-only Finance clearance before handover. Historical windscreen fields remain stored but are not part of the Delivery workflow. |
| Delivery release and outstation scheduling | Implemented | Showroom/outstation details live on the delivery plan; release requires exact schedule evidence, reconciled Finance clearance, and no unresolved invoice-update request, then creates an immutable terminal record. |
| Agent car ownership, monthly sold count, current process, boss overview | Implemented | Closing a vehicle lead as Sold records the Sales agent; Leads > Cars I’m Handling is server-scoped to that agent, while Boss/Admin can select an agent and view current process plus the combined current handoff of responsible department and next action. |
| Customer invoice detail: sales price, interest/additional charges, NCD, windscreen charges, outstation delivery date | Implemented | Finance owns invoice amounts and validation; the active Delivery schedule is authoritative for the read-only outstation delivery date. |
| Bank prepare document, checklist, invoice generation, payment follow-up, Pending/Approve/Disbursed, accounting export, nett price | Implemented | Finance tracks the manual checklist, payment follow-up, and internal reconciliation prerequisites; it exports a spreadsheet for staff to submit manually to AutoCount. |
...
| AI OCR, loan eligibility prediction, photo optimization, profit prediction, WhatsApp, accounting export | OCR implemented for first scope; other automation remains extension point | Uploaded IC, VOC, receipt, and invoice OCR uses Google Document AI and existing `OcrJobs`; Finance exports a manual accounting spreadsheet, while WhatsApp, loan eligibility prediction, photo optimization, and profit prediction remain extension scope. |
