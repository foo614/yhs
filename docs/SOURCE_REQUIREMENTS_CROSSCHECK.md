...
| Delivery booking inspection, schedule, PIC, notification, inspection, prepare document, report | Implemented | The selected local Docker workflow retains the ordered delivery statuses and detailed forms for PIC, schedule, inspection booking/report, preparation, and customer notice. |
| Polish, tinted, wash, final checklist | Implemented | The local Delivery detail and edit flows track these checks and enforce the ordered release transition. |
| Insurance, road tax, windscreen insurance | Implemented | Release requires delivery- and buyer-bound `Policy`, `RoadTaxReceipt`, and `WindscreenPolicy` evidence plus valid expiry dates. |
| Delivery release and outstation scheduling | Implemented | Showroom/outstation details live on the delivery plan; release requires all exact schedule evidence and reconciled Finance clearance, with server-owned actor/time and terminal status enforcement. |
| Agent car ownership, monthly sold count, current process, boss overview | Not in selected local slice | The newer Sales My Cars workboard is intentionally excluded to follow the approved local Docker interface. |
| Customer invoice detail: sales price, interest/additional charges, NCD, windscreen charges, outstation delivery date | Implemented | Finance owns invoice amounts and validation; the local payment workflow retains its outstation delivery date field. |
