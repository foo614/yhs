...
Each event contains only `id`, `kind`, `title`, `startDate`, `endDate`, `time`, and `status`. Delivery events use the delivery ID, plate number, scheduled date/time, and delivery status; cancelled deliveries are excluded. Approved leave and business trips appear only as staff availability (`Busy`), with no reason, medical information, trip location, or customer/Finance details. Busy events have null time and status.
...
| `POST` | `/api/collection-transactions/{id}/reconcile` | Reconcile a collection after the funds are confirmed. The recorder cannot reconcile their own collection, evidence must be linked to that exact collection, any NCD or nett-price variance must have maker-checker approval, and only reconciled collections reduce balance. |
