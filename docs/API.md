...
| --- | --- | --- | --- |
| `GET` | `/api/vehicles` | `Vehicles` | Full vehicle records for Boss/Admin and Sales, including `purchasePrice` (real acquisition price), optional `modifiedPurchasePrice` (internal adjusted cost used for margin/profit), and read-only `repairCost`: final repair-job cost when present, otherwise the intake refurbishment total. |
