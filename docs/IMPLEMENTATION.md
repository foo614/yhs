...
- The Leads module includes a separate Cars I’m Handling view. Sales staff see only their assigned vehicles, Sold this month, Cars in progress, current process, responsible department, and next action; Boss/Admin can select an agent. Desktop rows and mobile cards group Car, Current process, and Current handoff; the handoff combines responsible department and next action, with compact agent attribution visible only to Boss/Admin. A released car is complete only while Finance remains reconciled; after a correction it returns to the in-progress list with Finance as the owner of the next action.
