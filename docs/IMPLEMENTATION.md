...
Finance legacy forms await mutations before closing, retain entries and show the server reason after failure. Voucher approval/payment controls use their dedicated transition endpoints on desktop and mobile. Settlement, document, OCR-list, and Cash Custody load failures are visible and retryable rather than silently appearing empty; unavailable totals are labelled unavailable. Manual Loan creation now requires Boss/Admin at both the page and API; normal Loan staff read, update, bank-decision and document-check access remains unchanged. Finance is not added to the exceptional manual-create entry. Finance V2 cash is linked one-to-one from each partial pending collection to its custody record; acceptance reconciles that collection and rejection reverses its allocation atomically.
...
- The Leads module includes a separate Cars I’m Handling view. Sales staff see only their assigned vehicles, Sold this month, Cars in progress, current process, and a combined current handoff showing the responsible department and next action; Boss/Admin can select an agent and see a compact agent identity, including when a failed filter request leaves previous results visible. A released car is complete only while Finance remains reconciled; after a correction it returns to the in-progress list with Finance as the owner of the next action.
- A lead closed as Sold establishes the vehicle's Sales agent attribution. The server uses that attribution for Cars I’m Handling and the monthly sold count.
...
- Customer 360 presents timestamps in UTC+8 and keeps date-only values unchanged. A permission-aware customer summary and section links help staff scan the profile; vehicle selection narrows linked history without assigning customer-level records to an inferred purchase.
- Customer 360 is available in the sidebar for its existing authorized roles. Its labelled customer selector uses a responsive width and separate name/identifier lines in the dropdown so staff can distinguish customers with matching or long names.
- Customer 360 authorized uploads support filename/category filtering, newest-first pagination, and protected image thumbnails with full preview and original download. Missing document requirements remain available in a collapsed summary grouped by vehicle; this presentation does not change workflow gates or classify missing files as overdue.
- Customer 360 opens exact Loan and Delivery records through their existing ID routes. Vehicles, Finance, and Leads actions open their modules; those modules do not yet provide equivalent read-only record links. Receipts without a resolvable payment-to-vehicle link remain accessible under All vehicles and Unlinked records.
...
HR previews and prepares payroll drafts, enters period-specific employee/employer EPF, SOCSO and EIS amounts and PCB with an official calculation reference, and submits to Finance. A separate Finance reviewer approves before a separate Boss/Admin publishes. Published and finalized historical legacy payslips are immutable; drafts may be refreshed, clearing statutory amounts for rechecking. Legacy generated slips for open periods, or generated before their period ended, may be rebuilt into drafts for the new approval flow without locking attendance. Staff see their own published/legacy slips. Finance can review payroll without access to private HR leave/MC records.
