...
import { canMarkDeliveryReady, canMarkNotificationSent, canMarkTwoDayNoticeSent, canReleaseDelivery, deliveryCreateBlockReason, deliveryDocumentCategories, deliveryStatusLabel, filterDeliverySchedules, markDeliveryReady, markNotificationSent, markTwoDayNoticeSent } from "./delivery";
...
describe("delivery workflow helpers", () => {
  ...
  it("limits delivery uploads to delivery-owned handover document categories", () => {
    expect(deliveryDocumentCategories).toEqual(["InspectionReport", "DeliveryDocument", "HandoverPhoto", "SignedHandover", "Policy", "RoadTaxReceipt", "WindscreenPolicy"]);
  });
  ...
  it("filters deliveries by combined keyword, status, and API readiness", () => {
    ...
    const readiness: Record<string, DeliveryReleaseReadiness> = {
      "delivery-ready": { isReady: true, financeCleared: true, missingCategories: [], missingEvidence: [], expiredDocuments: [], evidence: [] },
      "delivery-blocked": { isReady: false, financeCleared: false, missingCategories: ["Policy"], missingEvidence: [], expiredDocuments: [], evidence: [] }
    };
  ...
  it("matches compact plates and the status labels shown to staff", () => {
    const deliveries = [
      { ...baseDelivery, id: "delivery-docs", status: "PreparingDocuments" as const },
      { ...baseDelivery, id: "delivery-ready", vehicleId: "vehicle-2", status: "ReadyForRelease" as const }
    ];

    expect(filterDeliverySchedules(deliveries, filterVehicles, {}, { keyword: "VPK1234" }).map((delivery) => delivery.id))
      .toEqual(["delivery-docs"]);
    expect(filterDeliverySchedules(deliveries, filterVehicles, {}, { keyword: "preparing docs" }).map((delivery) => delivery.id))
      .toEqual(["delivery-docs"]);
    expect(filterDeliverySchedules(deliveries, filterVehicles, {}, { keyword: "ready for release" }).map((delivery) => delivery.id))
      .toEqual(["delivery-ready"]);
  });

  it("keeps delivery status labels aligned with the visible workflow", () => {
    expect(deliveryStatusLabel("BookingInspection")).toBe("Booking inspection");
    expect(deliveryStatusLabel("PreparingDocuments")).toBe("Preparing docs");
    expect(deliveryStatusLabel("CarPreparation")).toBe("Car prep");
    expect(deliveryStatusLabel("ReadyForRelease")).toBe("Ready");
    expect(deliveryStatusLabel("Cancelled")).toBe("Cancelled");
  });
});
