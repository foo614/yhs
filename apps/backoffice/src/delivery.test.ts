...
const baseDelivery: DeliverySchedule = {
  ...
  windscreenPolicyReference: undefined,
...
describe("delivery workflow helpers", () => {
  ...
  it("allows release only when delivery, insurance, and road tax handover checks are ready", () => {
    ...
    expect(canReleaseDelivery({ ...readyDelivery, handoverPhotoCaptured: false })).toBe(false);
    expect(canReleaseDelivery({ ...readyDelivery, signedHandoverReceived: false })).toBe(false);
    expect(canReleaseDelivery({ ...readyDelivery, customerAcknowledged: false })).toBe(false);
    expect(canReleaseDelivery({ ...readyDelivery, finalChecklistConfirmed: false })).toBe(false);
    expect(canReleaseDelivery({ ...readyDelivery, insuranceExpiryDate: "2026-06-02" })).toBe(false);
    expect(canReleaseDelivery({ ...readyDelivery, roadTaxExpiryDate: undefined })).toBe(false);
  ...
  it("allows marking ready only after the delivery checklist is complete", () => {
    ...
    expect(canMarkDeliveryReady({ ...checklistComplete, customerAcknowledged: false })).toBe(false);
    expect(canMarkDeliveryReady({ ...checklistComplete, windscreenInsuranceExpiryDate: "2026-06-01" })).toBe(false);
  ...
  it("blocks manual ready-for-release submissions until the checklist is complete", () => {
    expect(deliveryCreateBlockReason({
      ...
      status: "ReadyForRelease",
      roadTaxHandled: false
    })).toBe("Delivery cannot be marked ready until inspection, inspection report, documents, car preparation, insurance, road tax, windscreen insurance, 2-day notice, release evidence, and current expiry dates are complete.");
  ...
  it("blocks manual released submissions until the checklist is complete", () => {
    expect(deliveryCreateBlockReason({
      ...
      status: "Released",
      windscreenInsuranceHandled: false
    })).toBe("Delivery cannot be released until inspection, inspection report, documents, car preparation, insurance, road tax, windscreen insurance, 2-day notice, release evidence, and current expiry dates are complete.");
