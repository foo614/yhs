...
export const deliveryDocumentCategories = ["InspectionReport", "DeliveryDocument", "HandoverPhoto", "SignedHandover", "Policy", "RoadTaxReceipt", "WindscreenPolicy"] as const;
...
export function deliveryStatusLabel(status: DeliverySchedule["status"]) {
  return ({
    ...
    Released: "Released",
    Cancelled: "Cancelled"
  } satisfies Record<DeliverySchedule["status"], string>)[status];
}
